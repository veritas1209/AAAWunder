...
def get_stack_dump(size=0x100):
    try:
        sp = gdb.parse_and_eval("$rsp")
    except:
        try:
            sp = gdb.parse_and_eval("$esp")
        except:
            return "Could not determine Stack Pointer."
    return gdb.execute(f"x/{size}gx {sp}", to_string=True)
...
def analyze_mode():
    ...
    try:
        # 1. starti
        print("[*] Executing 'starti'...")
        gdb.execute("starti")
        
        # 2. Memory Map
        print("[*] Dumping Memory Mappings...")
        mappings = run_shell_cmd(f"cat /proc/{gdb.selected_inferior().pid}/maps")
        ...
        try:
            ...
            print(f"\033[1;31m[!] Could not find 'main' symbol ({e}). Continuing from Entry Point.\033[0m")
        ...
        # 5. 어셈블리 덤프
        print("[*] Dumping Assembly Code (Main)...")
        try:
            asm_content = gdb.execute("disassemble", to_string=True)
            saved_chunks = save_chunks(f"{base_name}_wunder", asm_content, chunk_size=2000)
            print(f"    -> Split into {len(saved_chunks)} chunk(s).")
        except:
            print("    [!] Failed to disassemble current context.")

        # 6. [FIX 2] 끝까지 실행 (finish -> continue)
        print("[*] Running to end (continue)...")
        end_state = "Unknown"
        try:
            gdb.execute("continue")
            # 여기까지 코드가 도달했다면 프로그램이 종료되지 않고 멈춘 것 (Breakpoint 등)
            end_state = "Paused (Breakpoint or Step)"
        except gdb.error as e:
            # 보통 프로그램이 종료(Exited)되면 여기서 예외가 발생함 -> 정상적인 흐름
            msg = str(e)
...
[Memory Mappings]
Saved to {base_name}_maps.txt

[Assembly Dump]
Saved to {base_name}_wunder_partX.asm
...
"""
        ...
        print(f"\n\033[1;31m[!] Critical Error during analysis: {e}\033[0m")
...
def advanced_analyze():
    ...
    while True:
        ...
        if choice == '1':
            ...
            bp = input("Enter Address or Symbol (e.g., *0x401000 or main): ").strip()
            if bp: state.config_breakpoints.append(bp)
